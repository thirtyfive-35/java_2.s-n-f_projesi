...
public class KucukBas extends BaseAnimals {

	public KucukBas() {
		super.setHayvanSayısı(hayvanSayisiHesap());                   
		super.setSuDegeri(getHayvanSayısı() * 5);						
		super.setYemMiktari(getHayvanSayısı() * 5);						
		super.setYemUcreti(getYemMiktari() * 20);					
	...
	public boolean sicaklikKontrol() {
		...
		if (getSıcaklıkDeger() < 28 && getSıcaklıkDeger() > 15) {
	...
	public boolean nemKontrol() {
		...
		if (getNemYuzdesi() < 75 && getNemYuzdesi() > 50) {
	...
	public void temelBilYazdır() {
		System.out.println("\r\r\t*** KÜÇÜKBAŞ HAYVAN ***");
